...
class TestOtsu(unittest.TestCase):
    ...
    def test_mu_helper(self):
        a1 = np.arange(10)
        mu0_1, mu1_1 = mu_helper(a1 / np.sum(a1), 4, 0.4, 0.6)
        self.assertNotIsInstance(mu0_1, np.ndarray, msg='mu is a single value variable')
        # Note by tutors: there might be a bug with the next test (line 74)... or not. Check the studnon forum - we might
        # upload a corrected version of this file if needed.
